...
function SuccessfulAttack(game, order)
	if (order.PlayerID == nil or game.Game.PlayingPlayers[order.PlayerID].IsAI) then
		return;
	end
	local playerData = Mod.PlayerGameData;
	playerData[order.PlayerID].canGetCards = 1;
	Mod.PlayerGameData = playerData;
end

function CreateFortBuildOrder (order)
	...
	if (playerData[order.PlayerID].numCards == nil) then return; end;
	if (playerData[order.PlayerID].numCards <= 0) then return; end;

	--Deduct one fort from their total
	playerData[order.PlayerID].numCards = playerData[order.PlayerID].numCards - 1;
	Mod.PlayerGameData = playerData;
...
function CountUsedCards(game, ID)
--If it's the right turn to award forts, grant one to each player who's still alive in the game
	if (game.Game.PlayingPlayers[ID].IsAI) then return; end;
	local playerData = Mod.PlayerGameData;
	if (playerData[ID].otherCardsUsed == nil) then
		playerData[ID].otherCardsUsed = 1;
	else
		playerData[ID].otherCardsUsed = playerData[ID].otherCardsUsed + 1;
	end
	Mod.PlayerGameData = playerData;
end
...
function GiveCardPieces(game)
	...
	for _,gp in pairs(filter(game.Game.PlayingPlayers, function(gp) return not gp.IsAI; end)) do
		...
		if (playerData[gp.ID].canGetCards == 1) then
			playerData[gp.ID].canGetCards = 0;
			playerData[gp.ID].numPieces = playerData[gp.ID].numPieces + Mod.Settings.piecesPerTurn;
		end
		if (playerData[gp.ID].numPieces >= Mod.Settings.piecesInCard) then
			playerData[gp.ID].numCards = playerData[gp.ID].numCards + math.floor(playerData[gp.ID].numPieces / Mod.Settings.piecesInCard);
			playerData[gp.ID].numPieces = playerData[gp.ID].numPieces % Mod.Settings.piecesInCard;
...
function DiscardCards (game)
	...
	for _,gp in pairs(filter(game.Game.PlayingPlayers, function(gp) return not gp.IsAI; end)) do
		...
		if (playerData[gp.ID].numCards + playerData[gp.ID].otherCardsCount - playerData[gp.ID].otherCardsUsed > game.Settings.MaxCardsHold) then
			playerData[gp.ID].numCards = game.Settings.MaxCardsHold - playerData[gp.ID].otherCardsCount + playerData[gp.ID].otherCardsUsed;
		end
		if (playerData[gp.ID].numCards <0) then
			playerData[gp.ID].numCards = 0;
		end
		playerData[gp.ID].otherCardsUsed = 0;
	end
...
function Server_AdvanceTurn_Order(game, order, result, skipThisOrder, addNewOrder)
	if (order.proxyType == 'GameOrderCustom' and startsWith(order.Payload, 'BuildFort_')) then  --look for the order that we inserted in Client_PresentMenuUI
		--create order to build order at the end of the turn
		CreateFortBuildOrder (order);
	...
	if (order.proxyType == 'GameOrderAttackTransfer' and result.IsAttack) then
		...
		if (structures == nil) then goto continue; end;
		if (structures[WL.StructureType.ArmyCamp] == nil) then goto continue; end;
		if (structures[WL.StructureType.ArmyCamp] <= 0) then goto continue; end;
	...
	if (order.proxyType == 'GameOrderAttackTransfer') then
		...
		--If an attack is successful, player who made it gets right to receive card peices.
			SuccessfulAttack(game, order);
		end
	end
	if (startsWith(order.proxyType, 'GameOrderPlayCard') or order.proxyType == 'GameOrderDiscard') then
		CountUsedCards(game, order.PlayerID);
	end
end
function Server_AdvanceTurn_Start(game, addNewOrder)
--count how many cards player had at the start of the turn
	CountCards(game);
end
